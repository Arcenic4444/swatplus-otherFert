      ...
      !! added by lichen lang, mpdf_fert_parmas
      type mpdf_fert_parmas_rot
        integer :: num_pdfs = 0                                            ! number of conditions
        character(len=8) :: fer_num = "        "                           ! number of alternatives
        real :: tot_fer = 0.                                             ! number of actions
        real(8), dimension(:), allocatable :: centers                 ! max times lu change can occur
        real(8), dimension(:), allocatable :: time_windows                 ! max times lu change can occur
        real(8), dimension(:), allocatable :: weights                 ! max times lu change can occur
      end type mpdf_fert_parmas_rot

      type mpdf_fert_parmas
        character (len=8) :: name = "        "                                 ! name of the decision table
        type (mpdf_fert_parmas_rot), dimension(:), allocatable ::mpdf_fert_parmas_rot_db
      end type mpdf_fert_parmas
      type (mpdf_fert_parmas), dimension(:), allocatable, target :: mpdf_fert_parmas_db
